import"./bootstrap.min-f122c4cb.js";import"./admin-header-e186e282.js";const u=document.querySelector(".detailContent"),B=document.querySelector(".deleteBtn"),L=document.querySelector(".sendBtn");let a,m,P,v,r,o,f,b,x,g,y,w,h,S,$,k,q;const d=location.href.split("=")[1],s="https://dealmealserver.onrender.com",i=Swal.mixin({toast:!0,position:"top",showConfirmButton:!1,timer:1e3,timerProgressBar:!1,didOpen:e=>{e.onmouseenter=Swal.stopTimer,e.onmouseleave=Swal.resumeTimer}});T();function T(){axios.get(`${s}/products/${d}`).then(function(e){I(e.data)}).catch(function(e){console.log(e)})}function I(e){let t="",l="";e.series=="塑身系列"?l=`
    ...
    `,u.innerHTML=t,m=document.querySelector(".imgArea"),a=document.querySelector(".imgInput"),r=document.querySelector("#productName"),v=e.name,o=document.querySelector("#existNameText"),P=document.querySelector("#productSeries"),f=document.querySelector("#productPrice"),b=document.querySelector("#productIngredient"),x=document.querySelector("#productSize"),g=document.querySelector("#productCalories"),y=document.querySelector("#productCarb"),w=document.querySelector("#productProtein"),h=document.querySelector("#productFat"),S=document.querySelector("#microwave"),$=document.querySelector("#oven"),k=document.querySelector("#electricPot"),q=document.querySelector("#pan")}B.addEventListener("click",e=>{e.preventDefault(),Swal.fire({title:`確定要刪除登錄：<br>${v}？`,showDenyButton:!0,showCancelButton:!1,confirmButtonText:"確認",denyButtonText:"取消"}).then(t=>{t.isConfirmed&&D()})});function D(){axios.get(`${s}/carts`).then(function(e){e.data.forEach(t=>{let l=t.cart;for(let n=0;n<l.length;n++)l[n].productId==d&&l.splice(n,1);let p=t.id;axios.patch(`${s}/carts/${p}`,{cart:l}).then(function(n){}).catch(function(n){console.log(n)})})}).catch(function(e){console.log(e)}),axios.delete(`${s}/products/${d}`).then(function(e){i.fire({icon:"success",title:"刪除成功"}).then(t=>{window.location.href="admin-products.html"})}).catch(function(e){console.log(e)})}let c=!1,C=!1;u.addEventListener("click",function(e){e.preventDefault(),e.target.getAttribute("data-js")=="預覽圖片"&&(m.innerHTML=`
        <img class="" src="${a.value}" alt="">
        `,C=!0)});u.addEventListener("click",function(e){e.preventDefault(),e.target.getAttribute("data-js")=="移除圖片"&&(m.innerHTML=`
        <p class="text-danger">目前是空的</p>
        `,a.value="",C=!1)});u.addEventListener("click",function(e){e.preventDefault(),e.target.getAttribute("data-js")=="檢查名稱"&&(r.value||i.fire({icon:"warning",title:"商品名稱不可為空"}),N(r.value))});function N(e){axios.get(`${s}/products`).then(function(t){if(!e)o.classList.remove("text-success"),o.classList.add("text-danger"),o.textContent="名稱不可為空",c=!1;else{let l=!1;t.data.forEach(function(p){if(e==p.name&&e!=v){l=l||!0;return}else l=l||!1}),l?(o.classList.remove("text-success"),o.classList.add("text-danger"),o.textContent="此名稱已存在",c=!1):(o.classList.remove("text-danger"),o.classList.add("text-success"),o.textContent="此名稱可以使用",c=!0)}}).catch(function(t){console.log(t)})}function E(){axios.patch(`${s}/products/${d}`,{image:a.value,series:P.value,name:r.value,price:Number(f.value),ingredient:b.value,nutrition:{size:Number(x.value),calories:Number(g.value),carb:Number(y.value),protein:Number(w.value),fat:Number(h.value)},heat:{microwave:S.value,oven:$.value,electricPot:k.value,pan:q.value}}).then(function(e){i.fire({icon:"success",title:"成功送出"}).then(t=>{window.location.href="admin-products.html"})}).catch(function(e){console.log(e)})}L.addEventListener("click",function(e){e.preventDefault(),N(r.value),a.value?C?r.value?c?f.value?b.value?x.value&&g.value&&y.value&&w.value&&h.value?S.value&&$.value&&k.value&&q.value?Swal.fire({title:"確認送出？",showDenyButton:!0,showCancelButton:!1,confirmButtonText:"確認",denyButtonText:"取消"}).then(t=>{t.isConfirmed&&E()}):i.fire({icon:"warning",title:"加熱方式不可為空"}):i.fire({icon:"warning",title:"營養標示不可為空"}):i.fire({icon:"warning",title:"內容物不可為空"}):i.fire({icon:"warning",title:"單價不可為空"}):i.fire({icon:"warning",title:"請檢查商品名稱"}):i.fire({icon:"warning",title:"商品名稱不可為空"}):i.fire({icon:"warning",title:"請先預覽過圖片"}):i.fire({icon:"warning",title:"圖片網址不可為空"})});

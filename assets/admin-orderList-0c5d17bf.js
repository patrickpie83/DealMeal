import"./bootstrap.min-f122c4cb.js";import"./admin-header-e186e282.js";const n=document.querySelector(".orderStatusTitle"),u=document.querySelector(".orderListContent");document.querySelector(".moveOrderContent");document.querySelector(".moveOrderConfirmBtn");let c=location.href.split("=")[1];const f="https://dealmealserver.onrender.com",p=Swal.mixin({toast:!0,position:"top",showConfirmButton:!1,timer:1e3,timerProgressBar:!1,didOpen:r=>{r.onmouseenter=Swal.stopTimer,r.onmouseleave=Swal.resumeTimer}});c=="notProcessedOrder"?(c="尚未處理",n.textContent="尚未處理的訂單"):c=="processingOrder"?(c="揀貨中",n.textContent="揀貨中的訂單"):c=="deliveringOrder"?(c="出貨中",n.textContent="出貨中的訂單"):c=="canceledOrder"?(c="訂單取消",n.textContent="已取消的訂單"):c=="completedOrder"&&(c="訂單完成",n.textContent="已完成的訂單");h();function h(){axios.get(`${f}/orders`).then(function(r){k(r.data)}).catch(function(r){console.log(r)})}function k(r){let o="";r.forEach(function(e){if(e.status==c){let s="";e.status=="尚未處理"?s=`
                ...
                `);let d=e.cart,t="";for(let a=0;a<d.length;a++)a==d.length-1?t+=`${d[a].productName}X${d[a].quantity}`:t+=`${d[a].productName}X${d[a].quantity}、`;let l="";e.couponDiscount?l=`${e.couponDiscount}`:l="無",o+=`
            <tr>
                <td>${e.orderDate}</td>
                <td>${e.id}</td>
